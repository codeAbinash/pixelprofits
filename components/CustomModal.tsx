  ...
  return (
    <View style={{ width: '100%' }}>
      {modals.map((modal: any, i: number) => {
        ...
        const buttons = modal.buttons || [{ text: 'OK', positive: true }]
        return (
          <Modal key={i} animationType='fade' transparent={true} visible={modalActive}>
            <View
              style={{
                flex: 1,
                  ...
                  {buttons.map((btn: any, i: number) => {
                    return (
                      <TouchableOpacity
                        activeOpacity={0.8}
                        key={i}
                        style={{ width: buttons.length == 1 ? '100%' : '49%' }}
                        onPress={() => {
                      ...
                      >
                        <Text style={[modalStyles.modalButton, btn.positive ? modalStyles.buttonActive : {}]}>
                          {btn.text}
                        </Text>
                      </TouchableOpacity>
                    )
