...
const Onboarding = ({ navigation }: any) => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <FlatList
        data={data}
        renderItem={({ item }) => <BoardingItem item={item} key={item.key} />}
        horizontal
      ...
      <View style={[styles.bottom, { width: '100%' }]}>
        ...
        >
          ...
          <TouchableOpacity onPress={() => Linking.openURL(t_and_c_link)}>
            <Text style={[txt.color, styles.bottomText, { fontFamily: fonts.medium }]}>Terms and Conditions</Text>
          </TouchableOpacity>
...
function BoardingItem({ item }: { item: any }) {
  ...
  return (
    <View style={[{ width }, styles.boardingItem]}>
      ...
      >
        <Text style={[styles.title, { fontFamily: fonts.semiBold, textAlign: 'center', marginBottom: 10 }]}>
          {item.title}
        </Text>
