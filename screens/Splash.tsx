...
async function unexpectedLoggedOut(navigation: any, setModals: Function) {
  setModals([
    {
      title: 'Error',
      description: 'Unexpectedly logged out from the app. Please login again.',
    },
  ])
...
const Splash = ({ navigation }: any) => {
  async function mainProcess() {
    ...
    const updateFetch = await fetch(API_URL.get_update, {
      method: 'POST',
    ...
    if (isLoggedIn === 'true') {
      ...
      try {
        const fetched = await fetch(API_URL.get_user, {
          method: 'POST',
          headers,
        })
  ...
  return (
    ...
    >
      <StatusBar
        ...
        barStyle={statusBarColor === colors.accent ? 'light-content' : 'dark-content'}
      />
      <CustomModal modals={modals} updater={setModals} />
