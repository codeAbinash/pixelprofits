...
const EditProfile = ({ route, navigation }: any) => {
  ...
  React.useEffect(() => {
    const backAction = () => {
      ...
      setModals([
        {
          title: 'Are you sure?',
          description: 'Are you sure you want to cancel edit profile?',
          active: true,
          buttons: [
            { text: 'No' },
            {
  ...
  async function uploadPic() {
    if (!profilePic && !name && !email) {
      setModals([
        {
          title: 'Error',
          description: 'Please select a profile picture or enter name or email',
          active: true,
  ...
  return (
    <View className='flex-1 justify-between bg-white'>
      ...
      <View>
        <TouchableOpacity
          className='ml-auto mr-auto pb-10'
          activeOpacity={0.8}
          onPress={() => {
            selectPic()
          }}
        >
          <Image
            source={profilePic ? { uri: profilePic } : icons.user_icon}
            style={{
              width: 150,
              height: 150,
              borderRadius: 500,
      ...
      <View className='gap-4 p-5'>
        <View>
          <Text style={styles.label} className='mb-3'>
            Your Name
          </Text>
