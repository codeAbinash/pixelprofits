...
const HomeScreen = ({ navigation }: any) => {
  ...
  async function updateUserData() {
    ...
    // console.log(res)
    if (res.status === true || res.status === 'true') storeUserData(res)
    else {
    }
  }
  ...
  useEffect(() => {
    ...
    async function loadPopup() {
      ...
      if (res.data == null || data.status == 0) return
  ...
  return (
    <SafeAreaView style={{ paddingBottom: 50, backgroundColor: 'white', flex: 1 }}>
      <Modal visible={appAlert} animationType='fade' transparent={true} className='items-center justify-center'>
          ...
          <TouchableOpacity activeOpacity={0.8} onPress={() => navigation.navigate('Notifications')}>
            <View>
              <Image
                ...
                style={[styles.topImage, { width: 20, height: 20, opacity: 0.9 }]}
              />
    ...
    },
    {
      ...
      callback: () =>
        setModals([
          {
            title: 'Coming Soon',
            description: 'We are working on this feature. It will be available soon. Stay tuned!',
          },
        ]),
      // callback: () => {
                ...
                <View style={taskStyles.task}>
                  <Image
                    ...
                    style={[
                      taskStyles.image,
                      {
                        backgroundColor: task.background ? task.background : '#fff',
                      },
                    ]}
                  />
