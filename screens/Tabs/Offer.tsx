  ...
  async function getOfferStatus() {
    ...
    try {
      ...
      if (data.status === 'true' || data.status === true) {
        ...
        return fetch(API_URL.app_install_task, {
          method: 'POST',
          headers: headers,
        })
          .then((data) => data.json())
      ...
      Alert.alert(
        'There is some error while fetching offers status. Please check your internet connection. And try again later.',
      )
    }
  }
        ...
        setModals([
          {
            title: 'Please Wait',
            description:
              'This offer will take some time to be checked. Please kindly wait for some time. The credit will be added to your account soon.',
          },
        ])
      } else {
        ...
        setModals([
          {
            ...
            description:
              'Maybe you did noy complete 10 YouTube tasks continuously without any one task gap. Try again.',
          },
        ])
      }
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <Modal animationType='fade' transparent={true} visible={uerNameModal}>
        <View className='flex-1 items-center justify-center bg-[#00000033]'>
          <View className='w-[90%] rounded-2xl bg-white p-7'>
            <Text className='text-center text-lg text-[#000]' style={{ fontFamily: fonts.medium }}>
              Enter your telegram username
            </Text>
            <View className='mt-7 items-center justify-between'>
              <TextInput
                ...
                className='w-[100%] rounded-xl border-[#ccc] p-3 pl-4 text-base'
              ...
              <View className='mt-2 w-[105%] flex-row items-center justify-between gap-[4%]'>
                ...
                <View className='w-[47%]'>
                  <ButtonFull
                    title='Claim'
                    onPress={() => {
                      claimTelegramOffer(telegramUserName)
                    }}
                  />
                </View>
              </View>
              ...
              <View className='mt-2 w-[105%] flex-row items-center justify-between gap-[4%]'>
                ...
                <View className='w-[47%]'>
                  <ButtonFull
                    title='Claim'
                    onPress={() => {
                      submitAppInstallLink(appInstallLink)
                    }}
                  />
                </View>
              </View>
          ...
          <View className='w-[90%] rounded-2xl bg-white p-7'>
            ...
            <View className='mt-5'>
              <Image
                ...
                className='mx-auto h-20 w-20 rounded-full opacity-70'
              />
            </View>
            <Text className='mt-5 text-center text-lg text-[#000]' style={{ fontFamily: fonts.medium }}>
        ...
        <View className='gap-3 p-5'>
          ...
          {loadTaskData ? (
            offersStatus.map((item: OfferData, index: number) => {
              ...
              return (
                <View className='rounded-2xl bg-[#eeeeee] p-4 ' key={index}>
                  <View className='flex-row justify-between'>
                    <Text className='w-[70%] text-[#000]' style={{ fontFamily: fonts.medium }}>
                      {item.title}
                    </Text>
                    <TouchableOpacity
                      activeOpacity={!isActiveBtnClaim ? 1 : 0.7}
                      onPress={item.action}
                      disabled={!isActiveBtnClaim}
                    >
                      <View
                        className='rounded-xl p-3 px-4'
                        style={{
                          backgroundColor: colors.accent,
                          opacity: !isActiveBtnClaim ? 0.7 : 1,
                        }}
                      >
                        <Text className='text-white'>{getTaskStatusButtonText(item.status!)}</Text>
                      </View>
                    </TouchableOpacity>
                  </View>
                  {item.link && (
            ...
            <View className='pt-20'>
              <Loading />
              <Text className='mt-5 text-center text-lg text-[#000]'>Loading...</Text>
            </View>
          )}
        </View>
      </View>
