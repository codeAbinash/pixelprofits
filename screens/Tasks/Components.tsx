...
export function GoBtn({ url }: { url: string }) {
  return (
    <TouchableOpacity
      style={[buttons.full, { backgroundColor: 'limegreen', width: width / 3 - 25 }]}
      activeOpacity={0.8}
      onPress={() => {
        if (!url) return
        Linking.openURL(url)
...
export function getTaskStatus(status: string) {
  if (status === 'complete') return 'Task Completed Successfully'
  else if (status === 'processing') return 'Uploaded, Task Processing'
}

export function getTaskStatusColor(status: string) {
  if (status === 'complete') return 'limegreen'
  ...
  return (
    ...
    >
      ...
      <Text style={{ fontSize: 12, fontFamily: fonts.regular, color: colors.text }}>
        But you can retry again by clicking the 'Start Recording' button bellow
      </Text>
    </View>
  )
}
  ...
  return (
    <View
      style={{
        ...
        justifyContent: 'center',
        alignItems: 'center',
        height: isVisible ? 50 : 20,
      }}
    >
      <Animated.View
        style={{
          marginBottom: bottomSwipeIcon,
          marginTop: topSwipeIcon,
          gap: 2,
        }}
      >
        <Image
...
function getTimeInHoursMinutesSeconds(time: number) {
  ...
  if (isNaNInfinity(hours) || isNaNInfinity(minutes) || isNaNInfinity(seconds)) return '--:--:--'
...
export function Uploading({
  ...
  return (
    <View>
      {isError ? (
        <Text style={{ fontSize: 14, fontFamily: fonts.medium, color: 'red' }}>
          Network is busy, waiting for network connection.
        </Text>
      ) : (
        <Text style={{ fontSize: 14, fontFamily: fonts.medium, color: colors.text }}>
          Uploading {progress}% - {remainingTime} left
        </Text>
      )}
      <View
        style={{
