...
export default function Spin({ navigation }: any) {
  ...
  useEffect(() => {
    if (isSpinningFinished) setTextStatus('You have earned ' + earnedCoins + ' coins')
  }, [isSpinningFinished])
      ...
      fetch(API_URL.get_pin_coins, {
        ...
        .catch((err) => {
          setModalAlert([
            {
              title: 'Error',
              description: 'Network Error, Check your internet connectivity.',
              active: true,
              buttons: [
                {
                  text: 'Go Back',
  ...
  function spinWheel() {
    ...
    setRotate(
      rotateValue.interpolate({
        inputRange: [0, 1],
        outputRange: ['0deg', `${360 * 2 - randomDeg}deg`],
      }),
    )
    setTimeout(() => {
      setIsSpinning(false)
    ...
    >
                ...
                >
                  <View
                    style={{
                      ...
                      alignItems: 'center',
                      gap: 5,
                      transform: [{ rotate: `${index * deg + 90}deg` }],
                    }}
                  >
                    <Text
                      style={{
      ...
      <View
        style={{
          padding: 20,
          width: width,
          opacity: isSpinning || !isLoadedCoins ? 0 : 1,
        }}
      >
        {
          <ButtonFull
