...
const Wallet = ({ navigation }: any) => {
  ...
  useEffect(() => {
    setTimeout(async () => {
      ...
      const data = await fetch(API_URL.get_wallet_account, {
        method: 'POST',
        headers: headers,
      })
  ...
  async function withdrawAmount() {
    ...
    async function bindAccount() {
      ...
      const data = await fetch(API_URL.bind_wallet_account, {
        method: 'POST',
        headers: headers,
        body: JSON.stringify({
          type: accountType,
  ...
  async function getWithdrawHistory() {
    ...
    const headers = getDefaultHeader(await AsyncStorage.getItem('token'))
    const data = await fetch(API_URL.withdraw_history, {
      method: 'POST',
      headers: headers,
    })
    const res = await data.json()
        ...
        <View style={{ marginTop: isBound ? 0 : 30 }}>
          ...
          <View style={{ marginTop: 15, gap: 15 }}>
            {!isBound ? (
              ...
              <Text className='rounded-2xl bg-[#eeeeee] p-3 text-center text-base text-[#333333]'>
                ...
                return (
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      ...
                      setWithdrawCoins((item * INR_TO_COINS).toString())
                    }}
                  >
                    <View
                      ...
                      style={{
                        borderRadius: 30,
                        backgroundColor: selectedAmount === item ? colors.accent : colors.inputBg,
                        justifyContent: 'center',
                        alignItems: 'center',
                      }}
                    >
                      <Text
                        style={{
                          fontSize: 16,
                          color: selectedAmount === item ? 'white' : colors.text,
                          fontFamily: fonts.semiBold,
                        }}
                      >
                        ₹ {item}
            ...
            {withdrawCoins ? (
              ...
              <Text style={{ fontFamily: fonts.regular, color: colors.text }}>
                {' '}
                {inr_to_coins(+withdrawCoins, coins)}{' '}
              </Text>
            ) : null}
            <ButtonFull
              title={buttonText}
